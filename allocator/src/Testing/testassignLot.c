...
// Intializing the two structs for the parking lot and the car itself for the test case
    for (int i = 0; i < 10; i++) {
        lotArray[i].floor = 1;
...
TEST_CASE(deAssignSpace_test, {
    ...
    CHECK(car.assigned_lot == -1, "Car is deassigned from the space and returns -1");
...
TEST_CASE(reAssignSpace_test, {
    ...
    CHECK(lot.status == ERROR, "Lot status needs to be set to Error when reassigned");
...
TEST_CASE(checkOccupation_test, {
    ...
    CHECK(index == 3, "checkOccupation will skip over the first 3 lots, since they are occupied and return index 3");
})

MAIN_RUN_TESTS(assignLot_test, deAssignSpace_test, reAssignSpace_test,checkOccupation_test)
